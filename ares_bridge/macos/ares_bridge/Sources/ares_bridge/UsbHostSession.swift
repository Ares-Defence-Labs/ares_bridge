...
final class UsbHostSession {
  ...
  // Android's accessory gadget exposes a streaming bulk endpoint backed by
  // comparatively small kernel buffers. Keeping each host request bounded
  // prevents a large file chunk from monopolising the pipe until macOS's
  // synchronous request timeout expires.
  ...
  private let inputRequestBuffer = NSMutableData(
    length: 18 + UsbWireProtocol.maxHeaderBytes + UsbWireProtocol.maxPayloadBytes
  )!
  ...
  private func heartbeat() {
    ...
    let heartbeatState = stateLock.withLock { () -> (UsbHostError?, Bool) in
      ...
      let error = now - startedAtMs > Int64(configuration.peerTimeoutMs)
        ? UsbHostError.transport("The Android accessory handshake timed out.")
        : nil
      return (error, false)
    }
  ...
  private func send(_ data: Data) throws {
    ...
    while offset < data.count {
      ...
      {
        // IOUSBHost can report a timeout after making partial progress. Those
        // bytes are already visible to the accessory and must not be resent.
        NSLog(
        ...
        throw UsbHostError.transport("USB output request failed: \(friendly(error))")
      }
      guard transferred > 0 else {
        throw UsbHostError.transport("The USB output pipe stopped making progress.")
      }
      offset += transferred
    }
  }
  ...
  private func readExactly(_ count: Int) throws -> Data {
    ...
    while result.count < count, isRunning {
      ...
      // Android may submit an entire frame in one gadget-side write. A request
      // smaller than that write causes kIOReturnOverrun, so accept the largest
      // valid frame and decode individual protocol fields from the surplus.
      var transferred = 0
      do {
        try inputPipe.__sendIORequest(
          with: inputRequestBuffer,
          bytesTransferred: &transferred,
          completionTimeout: 1
        )
      } catch let error as NSError where error.code == Int(kIOReturnTimeout) {
        // A timed-out request may still have delivered bytes. Preserve them;
        // discarding partial progress would desynchronise every later frame.
        ...
        throw UsbHostError.transport("USB input request failed: \(friendly(error))")
      }
      guard transferred > 0 else {
        throw UsbHostError.transport("The USB input pipe closed.")
      }
      inputBuffer = Data(bytes: inputRequestBuffer.bytes, count: transferred)
      inputBufferOffset = 0
    }
